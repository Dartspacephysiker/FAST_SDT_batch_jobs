...
PRO SMOOTH_TO_4S_RES,printMe,var_name, $
                      IS_MAG=is_mag, $
                      MAGIND=magInd, $
                      ONESEC_TS=tS_1s, $
                     OUT_DATA=data
  ...
  delta_t_factor = 1.01
     ...
     ;; magInd = 1
     IF KEYWORD_SET(is_mag) THEN BEGIN
     ...
     FA_FIELDS_COMBINE,{TIME:tS_1s,COMP1:tS_1s}, $
                       ...
                       DELT_T=delta_t_factor/MIN(sRates), $
                       /TALK
...
PRO JOURNAL__20170519__STRANGEWAY_2000__FIGURE_5__ORB_8276, $
   TPLT_VARS=tPlt_vars, $
  ...
  PRINT,"STOP! This is incomplete! You were in the middle of trying to figure out whether you should smooth (i.e., use SMOOTH_TO_4S_RES) at all"
  PRINT,"If you were double-minded (see Brittany), the wiser of twain prevailed"
  STOP
...
; got mag data, set time limits, delete unused tplot variables, set tPlt_vars
     ...
     if KEYWORD_SET(use_fac) THEN magVar = 'dB_fac'

     if ~KEYWORD_SET(no_blank_panels) AND ~KEYWORD_SET(use_fac) then magVar = 'dB_fac_v'
     ...
     magInd = 1
     SMOOTH_TO_4S_RES,'SMOOTHmag',magVar, $
                      /IS_MAG, $
                      MAGIND=magInd, $
                      ONESEC_TS=tS_1s, $
                      OUT_DATA=data
     ...
     dBForPFluxVar = 'dB_fac_interp'
     ;; OPTIONS,dBForPFluxVar,'labels',(['o','e','b'])[magInd]

     STORE_DATA,dBForPFluxVar,DATA=data

     dLimit = {spec:0, ystyle:1, yrange:[-600., 800.], $
               ytitle:'dB Perp.!C!C[DC] (nT)', $
               panel_size:3}

     OPTIONS,dBForPFluxVar,'colors',[normColorI,normColorI]
     OPTIONS,dBForPFluxVar,'tplot_routine','mplot'
     STORE_DATA,dBForPFluxVar,DLIMITS=dLimit
...
; reset time limits if needed
     ...
     ;;Smooth to 4-s resolution
     SMOOTH_TO_4S_RES,'SMOOTHEfield','EFIT_ALONG_V', $
                      ;; /IS_MAG, $
                      ;; MAGIND=magInd, $
                      ONESEC_TS=tS_1s, $
                      OUT_DATA=data
...
; NOTE IT IS ASSUMED THAT FA_FIELDS_DESPIN DOES NOT CORRECT PHASE
     ...
     dEForPFluxVar = 'EFIT_ALONG_VSC'
     store_data,dEForPFluxVar,data=data,dlimit=dlimit
     options,dEForPFluxVar,'ytitle','E along V!Dsc!N!C!C[DC] (mV/m)'
     OPTIONS,dEForPFluxVar,'colors',[normColorI,normColorI]
     options,dEForPFluxVar,'panel_size',2
     ...
     if (n_elements(tPlt_vars) eq 0) then tPlt_vars=[dEForPFluxVar] else tPlt_vars=[dEForPFluxVar,tPlt_vars]
...
; Step 3 - Poynting flux
  GET_DATA,dBForPFluxVar,data=magData
  GET_DATA,dEForPFluxVar,data=eFData
...
; EFIT_ALONG_VSC
     ...
     bdat = where(tPlt_vars eq dEForPFluxVar,ndat)
     if (ndat eq 0) then begin
        t_arr = tlimit_all
        y_arr = [!values.f_nan,!values.f_nan]
        store_data,dEForPFluxVar, data={x:t_arr, y:y_arr}
        dlimit = {spec:0, ystyle:1, yrange:[-1000., 1000.], $
                  ytitle:'EFIT ALONG V!C!C55m (mV/m)', $
                  panel_size:3}
        store_data,'EFIT_ALONG_V',dlimit=dlimit
        options,dEForPFluxVar,'yrange',[-100.,100.]
        options,dEForPFluxVar,'ytitle','E along V!Dsc!N!C!C(mV/m)'
        options,dEForPFluxVar,'panel_size',2
     endif
...
;CHANGED to dB_fac
     ...
     ;; tPlt_vars=['SFA_V5-V8','DSP_V5-V8','Eesa_Energy','Eesa_Angle','Iesa_Energy','Iesa_Angle',dEForPFluxVar,'dB_fac_v']

     tPlt_vars=[dEForPFluxVar,dBForPFluxVar,'pFlux','Je','JEe','DSP_integ','Ji']
  endif
