...
; got mag data, set time limits, delete unused tplot variables, set tPlt_vars
     ...
     ENDIF ELSE BEGIN
        ...
        GET_MAG_TIME_INTERVALS_AND_SAVE,tx, $
                                        TIME_RANGES_OUT=time_ranges, $
                                        TIME_RANGE_INDICES_OUT=time_range_indices, $
                                        NINTERVALS_OUT=number_of_intervals
     ...
     ;;Clean up based on ILAT
     GET_FA_ORBIT,ephem_tSeries,/TIME_ARRAY,/DEFINITIVE,/ALL
...
; as 255 and we handle the call in the old way.
  ...
  FOR jj=0,number_of_intervals-1 DO BEGIN
     ...
     tmpPlotName  = outPlotName + '__magItvl_' + itvlString

     t1           = time_ranges[jj,0]
     t2           = time_ranges[jj,1]

     tmp_indices  = time_range_indices[jj,*]
     ...
     IF nKeep LT N_ELEMENTS(ephem_tSeries) THEN BEGIN
        ...
        closest1 = MIN(ABS(t1-ephem_tSeries[keep]),tmpII_t1)
        closest2 = MIN(ABS(t2-ephem_tSeries[keep]),tmpII_t2)
        ...
        t1             = ephem_tSeries[keep[tmpII_t1]]
        t2             = ephem_tSeries[keep[tmpII_t2]]
