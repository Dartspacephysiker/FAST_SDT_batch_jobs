;2016/09/08 Just trying to wrap my mind around these waves
PRO JOURNAL__20160908__REPRODUCE_MCFADDEN_ET_AL_1998A__ELECTRON_MODULATION__FIG_1, $
  ...
  do_losscone             = 0
  ;;Orbit 1881
  t1Str                   = '1997-02-11/09:11:30'
  t2Str                   = '1997-02-11/09:14:30'
  ...
  outPlotName             = 'McFadden_et_al_1998a__e-_modulation--Fig_1'

  ;;Get fields stuff, eFields and magFields
  V58 = GET_FA_FIELDS('V5-V8_S',t1,t2,/DEFAULT)
  STORE_DATA,'V5',DATA={x:V58.time,y:V58.comp1}
  OPTIONS,'V5','ytitle','E 55m!C!CmV/m'

  string = FA_FIELDS_SPEC(V58,/STORE,T_NAME='FFTV5',STRUCTURE=v58Spec,SLIDE=0.0)
  YLIM,'FFTV5',0.01,1.00,1        ; set y limits

  GET_DATA,'FFTV5',DATA=tmp
  tmp.y = tmp.y > 1e-9
  tmp.y = ALOG10(tmp.y)
  STORE_DATA,'FFTV5',DATA=TEMPORARY(tmp)
  ZLIM,'FFTV5',-7,1,0            ; set z limits
  OPTIONS,'FFTV5','ztitle','Log ' + v58Spec.units_name; z title
...
;; GET_2DT,'j_2d_b','fa_' + survOrBurst + '_c',NAME='JeF',T1=t1,T2=t2,ENERGY=energy_electrons,ANGLE=eAngle,/CALIB
  ...
  IF NOT KEYWORD_SET(dontShowPlots) THEN BEGIN
     ...
     TPLOT,['ion_180','ion_pa','el_0','el_pa', $
