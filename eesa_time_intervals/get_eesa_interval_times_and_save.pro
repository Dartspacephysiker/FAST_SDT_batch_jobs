PRO GET_EESA_INTERVAL_TIMES_AND_SAVE, $
   ...
   OVERWRITE_EXISTING_EESA_INTERVALS=overwrite_existing_eesa_intervals, $
   OVERWRITE_EXISTING_DIFF_EFLUX=overwrite_existing_diff_eFlux
  ...
  haveFile   = 0
  IF ~KEYWORD_SET(overwrite_existing_eesa_intervals) THEN BEGIN
     CD,defDir,CURRENT=curDir
     outFileTmp = STRING(FORMAT='(A0,I0,"--*_intervals.sav")', $
                         filePref,orbit_num)
     datafiles  = FILE_SEARCH(outFileTmp, /FOLD_CASE)

     CD,curDir
     IF N_ELEMENTS(dataFiles) GT 1 THEN BEGIN
        PRINT,"Too many files!"
        PRINT,dataFiles
        SPAWN,'cat "' + orbit_num + '" >> ' + logFile
        PRINT,"Returning ..."
        
     ENDIF ELSE IF ~STRMATCH('',dataFiles) THEN BEGIN

        IF KEYWORD_SET(get_diff_eFlux_too) THEN BEGIN
           outFile = (TEMPORARY(dataFiles))[0]

           PRINT,"Restoring " + outFile + ' ...'
           RESTORE,defDir+outFile

           haveFile = N_ELEMENTS(je                  ) NE 0 AND $
                      N_ELEMENTS(orbit_num           ) NE 0 AND $
                      N_ELEMENTS(time_range_indices  ) NE 0 AND $
                      N_ELEMENTS(number_of_intervals ) NE 0 AND $
                      N_ELEMENTS(time_ranges         ) NE 0

        ENDIF ELSE BEGIN
           PRINT,"Already have the desired file: " + outFile
           PRINT,'Exiting ...'
           RETURN
        ENDELSE
  ...
  IF KEYWORD_SET(get_diff_eFlux_too) THEN BEGIN
     ...
     ELECTRON_SPEC_IDENTIFICATION_V4__JUST_ELECTRONS_V2, $
        ...
        OVERWRITE_EXISTING=overwrite_existing_diff_eFlux, $
        /PROVIDING_JE_TIMES, $
        ORBIT_NUM=orbit_num, $
        JE=je, $
