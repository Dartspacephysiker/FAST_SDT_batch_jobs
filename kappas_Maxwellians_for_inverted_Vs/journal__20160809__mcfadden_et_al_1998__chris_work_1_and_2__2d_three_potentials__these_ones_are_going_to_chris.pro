...
;
PRO JOURNAL__20160809__MCFADDEN_ET_AL_1998__CHRIS_WORK_1_AND_2__2D_THREE_POTENTIALS__THESE_ONES_ARE_GOING_TO_CHRIS, $
  ...
  IF KEYWORD_SET(diff_eFlux_file) THEN BEGIN
     IF SIZE(diff_eFlux_file,/TYPE) NE 7 THEN BEGIN
        diff_eFlux_file     = '~/software/sdt/batch_jobs/saves_output_etc/orb_1849--diff_eflux--eeb--output_from_get_losscone_and_eflux_data.sav'
        ENDIF
  ...
  inSaveFile         = '20160809--McFadden_et_al_1998_Fig_1--four_currents--2dfits' + paredSuff + '.sav'
  ...
  IF KEYWORD_SET(save_kappa_gauss_corr) THEN BEGIN
     ...
     txtFile         = GET_TODAY_STRING(/DO_YYYYMMDD_FMT) + outSuff + '--linear_fits--kappa_and_gauss--orbit_1849.txt'
