...
;
PRO JOURNAL__20160809__ELPHIC_ET_AL_1998__CHRIS_WORK_1_AND_2__2D_THREE_POTENTIALS__THESE_ONES_ARE_GOING_TO_CHRIS, $
   ...
   CALC_OBS_CURRENT_FROM_DIFF_EFLUX_FILE=diff_eFlux_file, $
   ...
   SAVE_KAPPA_GAUSS_CORR=save_kappa_gauss_corr, $
   RBCORRLUN=RBCorrLun
  ...
  electron_aRange     = (N_ELEMENTS(calc_fitDens__aRange) EQ 2 ? calc_fitDens__aRange : eAngleChare)
  ...
  IF KEYWORD_SET(diff_eFlux_file) THEN BEGIN
     IF SIZE(diff_eFlux_file,/TYPE) NE 7 THEN BEGIN
        diff_eFlux_file     = '~/software/sdt/batch_jobs/saves_output_etc/orb_1773--diff_eflux--ees--output_from_get_losscone_and_eflux_data.sav'
        ENDIF

     RESTORE,diff_eFlux_file
     IF N_ELEMENTS(Jdiff_eFlux) EQ 0 THEN BEGIN
        nDECurrent         = N_ELEMENTS(diff_eFlux.time)

        Jdiff_eFlux = MAKE_ARRAY(nDECurrent,VALUE=0.0)
        FOR k=0, nDECurrent-1 DO BEGIN
           Jdiff_eFlux[k] = J_2D_FS(MAKE_SDT_STRUCT_FROM_PREPPED_EFLUX(diff_eFlux,k), $
                                           ANGLE=electron_aRange,ENERGY=energy_electrons)
        ENDFOR
        Jdiff_eFlux *= (-1.)*1.6e-9 ;;in microA/m2
     ENDIF
  ENDIF
  ...
  fitFile            = '20160809--Elphic_et_al_1998--Kappa_fits_and_Gauss_fits--ees--fit2d--all_times--150to150--mpfitfun1d--fresh.sav'

  inSaveFile         = '20160809--Elphic_et_al_1998_Fig_1--four_currents--2dfits' + paredSuff + '.sav'

  ;;outFiles
  SET_PLOT_DIR,plotDir,/FOR_SDT,/ADD_TODAY
  outPlotPref        = GET_TODAY_STRING(/DO_YYYYMMDD_FMT) + '--Elphic_et_al_1998--model_vs_obs--two_panels--2dfits'+outSuff

  ;;Comes with fit2dgauss_inf_list, fit2dkappa_inf_list, gaussfits, je, jee, kappafits, synthpackage
  RESTORE,saveDir+fitFile

  IF KEYWORD_SET(save_kappa_gauss_corr) THEN BEGIN
     print_kappa_gauss_corr = 1

     SET_TXTOUTPUT_DIR,txtOutputDir,/FOR_SDT,/ADD_TODAY 
     txtFile         = GET_TODAY_STRING(/DO_YYYYMMDD_FMT) + outSuff + '--linear_fits--kappa_and_gauss--orbit_1773.txt'
     OPENW,RBCorrLun,txtOutputDir+txtFile,/GET_LUN
  ENDIF
  ...
  setup = {kappaS:Astruct, $
           ...
           Je:(KEYWORD_SET(Jdiff_eFlux) ? Jdiff_eFlux : Je_kappa_interp), $
           Ji:Ji_kappa_interp}

  IF KEYWORD_SET(Jdiff_eFlux) THEN BEGIN
     setup = CREATE_STRUCT(setup,'Jdiff_eFlux',Jdiff_eFlux)
  ENDIF
  ...
  SETUP_POTENTIAL_AND_CURRENT,setup, $ 
                              ...
                              USE_JE_CURRENT=use_je_current, $
                              USE_JMAG_CURRENT=use_jMag_current, $
                              USE_CALCKED_JDIFF_EFLUX=diff_eFlux_file
  ...
  kappaDens = KAPPA__SELECT_2DFIT_DENS(kappa2D, $
                                       ...
                                       ELECTRON_ANGLERANGE=electron_aRange, $
