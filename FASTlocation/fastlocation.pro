pro fastlocation,filename=filename,energy_electrons=energy_electrons,energy_ions=energy_ions,analyse_noise=analyse_noise,$
  ...
  fastlocation_dir = '/SPENCEdata/software/sdt/batch_jobs/FASTlocation/'
...
;  IF NOT KEYWORD_SET(skip_if_file_exists) THEN skip_if_file_exists=1
  ...
  for jjj=0,number_of_intervals-1 do begin
     ...
     curfile = fastlocation_dir + 'batch_output/'+'Dartmouth_fastlocation_dflux_'+strcompress(orbit_num,/remove_all)+'_'+strcompress(jjj,/remove_all)
     IF KEYWORD_SET(burst) THEN BEGIN
...
;;filename='/SPENCEdata/software/sdt/batch_jobs/Alfven_study/fastlocation_14F/'+'Dartmouth_fastlocation_dflux_'+strcompress(orbit_num+'_'+string(jjj)+"_magcal_v"
