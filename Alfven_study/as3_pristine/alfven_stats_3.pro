pro Alfven_Stats_3,filename=filename,energy_electrons=energy_electrons,energy_ions=energy_ions,analyse_noise=analyse_noise,$
...
;**09/16/2014 
;Now I'm having issues with the loss cone calculation. I'm going to
;just let the loss cone be ~40 deg for now...
;There's an issue with my "loss_cone_width" function that I don't
;quite understand--some floating point error.
;The function oddly seems to work if I just call it within the IDLDE
;for values identical to those given in Alfven_Stats_3
;For orbit 10000, 
;print, "loss_cone_alt", loss_cone_alt, "ilat.y(0)", ilat.y(0)
;;loss_cone_alt       2103081.9ilat.y(0)       63.545739

;;This line provides an error:
...
;**09/16/2014
;OK, I think I see why this wasn't working. The original call
;to FA_FIELDS_COMBINE that I was using didn't include the
...
;*Edited call to FA_FIELDS_COMBINE to align efield data with magz times.
;Variable utilized in program used to be fields, but I've replaced with efield_aligned 
...
if jjj GT 0 or not keyword_set(filename) then begin
   filename='/SPENCEdata/software/sdt/batch_jobs/Alfven_study/as3_pristine/'+'Dartmouth_dflux_'+strcompress(orbit_num+'_'+string(jjj),/remove_all)
endif
