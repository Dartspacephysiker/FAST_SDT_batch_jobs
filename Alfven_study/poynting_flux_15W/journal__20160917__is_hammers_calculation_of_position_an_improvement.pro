;;09/17/16
PRO JOURNAL__20160917__IS_HAMMERS_CALCULATION_OF_POSITION_AN_IMPROVEMENT, $
...
;     RETURN
  ...
  FOR jjj=0,number_of_intervals-1 DO BEGIN
     ...
     IF (sc_pot.valid) AND (magDC.valid) THEN BEGIN
        ...
        FOR k=0,FFTCount-1 DO BEGIN
           ...
           IF k LT (FFTCount-1) THEN BEGIN
              WHILE ( fftBin_i[0,k+helper+1] EQ (fftBin_i[1,k+helper]+1) ) AND $
                 ( ( k+helper ) LT FFTCount-2  ) DO BEGIN
        ...
        IF KEYWORD_SET(full_pFlux) AND KEYWORD_SET(include_E_near_B) THEN BEGIN
        ...
        ;;Try hammer's way
        speed_point_inds = VALUE_CLOSEST2(vel.x,magz.x)
        smp2             = speed[speed_point_inds]
        pos2[1:-1] = TOTAL((magz.x[1:-1]-magz.x[0:-2])*smp2,/CUMULATIVE)
