     ...
     IF KEYWORD_SET(extra_t) THEN BEGIN
        for jj=0L,n_elements(keep)-1 do begin
           printf,unit1,format='(I9,A24,27G13.6,2A24,G13.6)',current_intervals(jj,19),time_to_str(current_intervals(jj,20),/ms),$
                  ...
                  current_intervals(jj,32),current_intervals(jj,33),current_intervals(jj,34),time_to_str(current_intervals(jj,35),/ms),$
                  time_to_str(current_intervals(jj,36),/ms),current_intervals(jj,37)
        endfor
     ENDIF ELSE BEGIN
        for jj=0L,n_elements(keep)-1 do begin
           ...
           IF current_intervals(jj,3) NE 0.0 THEN BEGIN
              fname='plots/current_orb10000_'+str(jj)
