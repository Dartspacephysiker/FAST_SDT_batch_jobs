PRO ALFVEN_STATS_5__LATERORBS,filename=filename,energy_electrons=energy_electrons,energy_ions=energy_ions,analyse_noise=analyse_noise,$
  ...
  as5_dir = '/SPENCEdata/software/sdt/batch_jobs/Alfven_study/as5--add_later_orbs/'

;  IF NOT KEYWORD_SET(cont_if_file_exists) THEN cont_if_file_exists=1

  ;;2016/04/30 Forked from alfven_stats_5. Trying to implement Jack Vernetti's wisdom, which you will find in
  ;;${SDT_DIR}/batch_jobs/UIcfgs_for_fields_summary_from_JackV/Jack_notes_on_whats_wrong_with_eesa_data_in_later_orbits.pdf
...
;     return
  ...
  t1             = 0.0D
  t2             = 0.0D
  startDat       = GET_FA_EES(t1,INDEX=0.0D)
  stopDat        = GET_FA_EES(t2,INDEX=DOUBLE(last_index))
