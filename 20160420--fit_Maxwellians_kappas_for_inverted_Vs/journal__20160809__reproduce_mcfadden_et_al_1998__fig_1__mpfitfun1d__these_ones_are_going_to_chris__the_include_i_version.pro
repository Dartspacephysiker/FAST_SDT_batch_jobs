;;08/09/2016 Time to send these friggin' things off
;;Runme with:
; JOURNAL__20160809__REPRODUCE_MCFADDEN_ET_AL_1998__FIG_1__MPFITFUN1D__THESE_ONES_ARE_GOING_TO_CHRIS,/SAVE_DATA,/SAVE_PS,FIT2D__CALC_FITDENS_OVER_ARANGE=[-150,150]
PRO JOURNAL__20160809__REPRODUCE_MCFADDEN_ET_AL_1998__FIG_1__MPFITFUN1D__THESE_ONES_ARE_GOING_TO_CHRIS__THE_INCLUDE_I_VERSION, $
   SAVE_DATA=save_data, $
   SAVE_PNG=save_png, $
   SAVE_PS=save_ps, $
  ...
  R_B                = 20.0      ;For calculating Maxwellian and Kappa current
...
; OPTIONS,'ion_pa','ztitle','Log Eflux!C!CeV/cm!U2!N-s-sr-eV' ; z title
  ...
  ;; IF KEYWORD_SET(excludeG_t) AND KEYWORD_SET(excludeK_t) THEN BEGIN
  ;;    exclude_i = CGSETINTERSECTION(excludeG_i,excludeK_i)
  ;; ENDIF ELSE BEGIN
  ;;    CASE 1 OF
  ;;       KEYWORD_SET(excludeG_t): BEGIN
  ;;          exclude_i = excludeG_i
  ;;       END
  ;;       KEYWORD_SET(excludeK_t): BEGIN
  ;;          exclude_i = excludeK_i
  ;;       END 
  ;;    ENDCASE
  ;; ENDELSE
  ;; IF N_ELEMENTS(exclude_i) GT 0 THEN BEGIN
  ;;    include_i = CGSETDIFFERENCE(include_i,exclude_i,COUNT=nExcluded)
  ;; ENDIF
  ...
  STORE_DATA,'kappa_fit',DATA={x:kappaTime[include_i],y:Astruct.kappa[include_i]}
  ...
  kappaStr                       = {time:kappaTime[include_i], $
                                    comp1:aStruct.kappa[include_i], $
                                    ncomp:1}
  ...
  STORE_DATA,'onecheese',DATA={x:kappaTime[include_i], $

                               y:obs_current[include_i]}
  STORE_DATA,'fourcheese',DATA={x:jMag.x, $

                                y:jMag.y}
  STORE_DATA,'toppings',DATA={x:[[kappaStr.time[include_i]],[kappaStr.time[include_i]]], $

                              y:[[gauss_current[include_i]],[kappa_current[include_i]]]}
  ...
  IF KEYWORD_SET(output_fit_textFile) OR KEYWORD_SET(print_current_info) THEN BEGIN
     ...
     PRINT_KAPPA_FITS,chartot_kappa_interp[include_i], $
                      kappa_current[include_i], $
                      kappaFits, $
                      OUTFILE=kappaTxt, $
                      OUTDIR=outDir

     PRINT_KAPPA_FITS,chartot_kappa_interp[include_i], $
                      gauss_current[include_i], $
                      gaussFits, $
                      OUTFILE=gaussTxt, $
                      OUTDIR=outDir
