...
PRO JOURNAL__20160712__REPRODUCE_ELPHIC_FIG_1__INCLUDE_FOUR_CURRENTS,SAVE_PNG=save_png,SAVE_PS=save_ps, $
   SAVE_DATA=save_data, $
   OUTPUT_FIT_TEXTFILE=output_fit_textfile, $
   PRINT_CURRENT_INFO=print_current_info

  COMPILE_OPT IDL2

  R_B                         = 20 ;For calculating Maxwellian and Kappa current
  ...
  outSaveFile                 = GET_TODAY_STRING(/DO_YYYYMMDD_FMT) + '--Elphic_et_al_1998_Fig_1--four_currents.sav'
  ...
  FA_FIELDS_COMBINE,kappaStr,{time:Jee.x,comp1:chartot,ncomp:1}, $
                    RESULT=chartot_interp,/INTERP,DELT_T=50.,/TALK
  ...
  IF KEYWORD_SET(save_data) THEN BEGIN
     ...
     SAVE,chartot_interp,R_B,chari,chare,kappaStr,kappa_current,maxwell_current,jESA,jMag, $
          Je_kappa_interp,Ji_kappa_interp,Jtot_kappa_interp,jMag_kappa_interp,FILENAME=saveDir+outSaveFile
