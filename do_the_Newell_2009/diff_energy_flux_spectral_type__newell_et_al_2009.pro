...
;-
PRO DIFF_ENERGY_FLUX_SPECTRAL_TYPE__NEWELL_ET_AL_2009,eSpec,Je,Jee,Ji,Jei,MLT, $
  ...
  event = { time:UTC_seconds, $
            eSpec_problems:0B, $     ;0 = no problems      , N = N bad bins
            monoenergetic:0B, $      ;0 = not monoenergetic, 1 = monoenergetic, 2 = strict_monoenergetic, (NEGATIVE) = step where algorithm failed
            broadband:0B, $          ;0 = not broadband    , 1 = broadband    , 2 = strict_broadband
            diffuse:0B, $            ;0 = not diffuse      , 1 = diffuse      , 2 = diffuse, flux extrapolated to 50 keV
            Je:DOUBLE(0), $          ;Electron number flux (#/cm^2-s)
            Jee:DOUBLE(0), $         ;Electron energy flux (mW/m^2)
            Ji:DOUBLE(0), $          ;Ion number flux      (#/cm^2-s)
            Jei:DOUBLE(0)}           ;Ion energy flux      (mW/m^2)

  ;;Check spectrum integrity
  bad_i      = WHERE(~FINITE(spectrum),nBad)
  IF nBad GT 0 THEN BEGIN
     IF nBad GT 255 THEN BEGIN
        PRINT,'nBad is too big to be assigned to a byte variable! Beware!'
        IF KEYWORD_SET(batch_mode) THEN BEGIN
           nBad = 255
        ENDIF ELSE BEGIN
           STOP
